...
function electronBuilder(electronArgs, context) {
    ...
    const firstBuild = ngBuild.pipe(operators_1.first(), operators_1.map(({ value: { baseUrl } }) => {
        electraLog(`Electron is watching ${baseUrl}. Start building...`);
        return baseUrl;
    }), operators_1.switchMap(baseUrl => watchElectron(electronArgs, context).pipe(operators_1.tap(() => electraLog('Build finished. Start app...')), operators_1.mergeMap(runElectron.call(this, electronArgs, baseUrl, context)))), operators_1.tap(code => electraLog(`Process end with exit code ${code}`)));
